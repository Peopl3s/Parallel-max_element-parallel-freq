...
template <typename Iterator>
Iterator parallel_min_element(Iterator first, Iterator last, atomic<bool> &done) 
{
    try {
        unsigned long const length=distance(first,last);
        unsigned long const min_per_thread=10;
        ...
        if(length<(2*min_per_thread)) 
	{
                ...
                if (*first < *smallest) 
		{
            ...
            future<Iterator> async_result=async(&parallel_min_element<Iterator>,mid_point,last,ref(done));
            Iterator const direct_result=parallel_min_element(first,mid_point,done);
...
template <typename Iterator>
Iterator parallel_min(Iterator first,Iterator last)
{
    atomic<bool>done(false);
    return parallel_min_element(first,last,done);
}
